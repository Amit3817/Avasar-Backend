...
export const sendOtp = async (mobile, otp) => {
  try {
    const API_KEY = process.env.TWO_FACTOR_API_KEY; // put your API key in .env
    const TEMPLATE_NAME = "Avasar"; // your approved template name

    const url = `https://2factor.in/API/V1/${API_KEY}/SMS/+91${mobile}/${otp}/${TEMPLATE_NAME}`;
    ...
    // Retry logic (same as your email version)
    const maxRetries = 3;
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
